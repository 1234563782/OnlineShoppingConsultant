...
- `catalog-mcp-server` (8083): product search/detail tools.
...
4. Orchestrator routes to `consult_agent` through A2A.
5. Consult agent calls MCP tools through Nacos-discovered tool callbacks.
6. Orchestrator appends turns back to Redis and returns reply.
